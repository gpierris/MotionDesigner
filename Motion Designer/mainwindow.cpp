...
void MainWindow::createActions()
{
	...
	connect(m_ui->insertPoseButton, SIGNAL(clicked()), this, SLOT(insertPoseAction()));
        connect(m_ui->undoButton, SIGNAL(clicked()), this, SLOT(undoAction()));
        connect(m_ui->redoButton, SIGNAL(clicked()), this, SLOT(redoAction()));

        connect(m_ui->moveUpPoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
        connect(m_ui->moveDownPoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
        connect(m_ui->swapPoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
        connect(m_ui->removePoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
        connect(m_ui->storePoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
        connect(m_ui->insertPoseButton, SIGNAL(clicked()), this, SLOT(saveState()));
...
void MainWindow::saveState(){
    ...
    unsigned int maxUndos = 20;
    std::vector<QString> tempState;
    int rows;

    if(poseEditorState.size() >= maxUndos){
    ...
    {
        tempState.push_back(compileMessage(i));
    }

    poseEditorState.push_back(tempState);
...
void MainWindow::undoAction(){
    ...
    std::vector< QString > tempState;
    QStringList tempPoseList;


    if(poseEditorState.size() <= 1){
        return;
    }
    ...
    tempState = poseEditorState.at(poseEditorState.size()-2);
    currentState = poseEditorState.size()-2;
    //poseEditorState.pop_back();

    for(unsigned int pose = 0; pose < tempState.size(); pose++){

        int lastRow = m_ui->poseEditor->rowCount();
        m_ui->poseEditor->insertRow(lastRow);

        tempPoseList = tempState.at(pose).split(QString("%"));

        for (int i = 0; i < robot->numOfJoints; i++)
        {

            QTableWidgetItem * item = new QTableWidgetItem(tempPoseList.at(i+1));
            item->setTextAlignment(Qt::AlignCenter);
...
void MainWindow::redoAction(){
    ...
    if(currentState==0 || currentState>=poseEditorState.size()){
    ...
    tempState = poseEditorState.at(currentState+1);
    currentState++;
    //poseEditorState.pop_back();
